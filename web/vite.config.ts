...
export default defineConfig(({ mode }) => {
  // The API dev host. Override it in a `web/.env` file (copy from .env.example) or via a shell env
  // var — loadEnv(mode, cwd, '') reads every key from .env files and merges the shell env on top.
  // Run the API with: dotnet run --project src/Dimes.Api  (binds http://localhost:5080 by default).
  const env = loadEnv(mode, process.cwd(), '')
  const API = env.DIMES_API ?? 'http://localhost:5080'
